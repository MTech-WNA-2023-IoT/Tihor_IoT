...
def mysql_push(data):
  ...
  cur.execute("INSERT INTO apiData(name, region, country, temperature_c, temperature_f, is_day, w_Condition, wind_speed_mph, wind_speed_kph, humidity) VALUES (%(name)s, %(region)s, %(country)s, %(temperature_c)s, %(temperature_f)s, %(is_day)s, %(condition)s, %(wind_speed_mph)s, %(wind_speed_kph)s, %(humidity)s);", data)
  conn.commit()
